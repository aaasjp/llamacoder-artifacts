...
import { dependencies } from "@/components/code-runner-react";

// 从 code-runner-react.tsx 的 dependencies 对象获取依赖库列表
function generateDependenciesList() {
...
- 跳过代码示例和评论。也不要包含任何外部API调用。不要包含任何外部API调用。
- 确保实现可以适合一个大的React组件。
